...
task.spawn(function()
	...
	player.OnTeleport:Connect(function()
		if queueteleport then
			queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/mamalolslolo-web/DFAttemptLol/refs/heads/main/obfuscated_script-1786725630658.lua'))()")
		end
	end)
end)
...
local function aimCameraAtEyes()
	...
	if eyes and hrp and motorRemote then
		...
		warn("Missing Eyes, HRP, or MotorReplication remote")
	end
end
		...
		if newCharacter and newHRP then
			...
			if room1 and roomExitPart then
				...
				if currentCharacter and currentCharacter.Parent then
					...
					if currentHRP then
						local exitPos = roomExitPart.Position
						print("Teleporting to RoomExit exact position:", exitPos)
						currentHRP.CFrame = CFrame.new(exitPos)
						print("RoomExit teleport completed")
								...
								if liveCharacter and liveCharacter.Parent then
									...
									if liveHRP and liveHRP.Parent then
										...
										if liveExitPart then
											local targetPos = liveExitPart.Position
											if (liveHRP.Position - targetPos).Magnitude > 2 then
												liveHRP.CFrame = CFrame.new(targetPos)
											end
										end
									end
								end
						...
						warn("HumanoidRootPart disappeared after revive.")
					end
				else
					warn("Current character disappeared after revive.")
				end
			else
				warn("Could not find Room 1 / RoomExit after revive.")
				if not room1 then
					warn("Room 1 was not found.")
				elseif not roomExitPart then
					warn("Room 1 exists, but no RoomExit/BasePart was found.")
				end
			end
		else
